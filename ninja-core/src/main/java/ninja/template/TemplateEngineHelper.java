...
public class TemplateEngineHelper {
    ...
    public String getTemplateForResult(Route route,
                                              Result result,
                                              String suffix) {
        if (result.getTemplate() == null) {
            Class<?> controller = route.getControllerClass();

            String parentControllerPackageAsPath = getParentPackage(controller);
    ...
    private String getParentPackage(Class<?> controller) {
        ...
        String parentPackageOfController = controllerPackageName.replaceFirst(
                NinjaConstant.CONTROLLERS_DIR, NinjaConstant.VIEWS_DIR);

        // And now we rewrite everything from "." notation to directories /
        String parentControllerPackageAsPath = parentPackageOfController
                .replaceAll("\\.", "/");
        return parentControllerPackageAsPath;
    }
